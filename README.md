...
> ベータ版 v0.1.0-beta.1 — 活発に開発中です。今後のアップデートで、設定形式や使い方が変わることがあります。
>
...
PocketRole は MIT License のもとで、現状有姿・無保証で提供されます。

本ソフトウェアの利用、生成されたコンテンツ、外部 LLM/API、公開 Web サーバーの利用に伴う損害・費用・データ公開・その他のトラブルについて、作者は法令で認められる範囲で責任を負いません。API キー、公開データ、各サービスの利用条件は、利用者自身の環境に合わせて管理してください。
...
- **状態**: ベータ — 機能は動作しますが、今後のアップデートで API・設定形式・操作手順が変わることがあります
